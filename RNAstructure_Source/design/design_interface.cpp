...
int designInterface::run() {
	...
	if(preselectSequences && !isRNA) {  //The program does not yet have pre-selected sequences for DNA
	...
	design *des; 
	des = new design(ctFile.c_str(), isRNA);//Read the structure into the 'design' class 'des'
	...
	errNum = des->design_sequence(
		defectThreshold,!preselectSequences,maxDepth,
		/*useHeuristicMode*/ false,maxRedesign,maxMutate,
		maxLeafRedesign, randSeed); 
	if (errNum != 0) goto exitProgram; //At bottom of function. deletes des and returns errNum.
	//
	/////END "Run Design"/////////////////////////////
	//////////////////////////////////////////////////
	...
	//	RNA *rna=new RNA(ctFile.c_str(),1,isRNA);
	//	RNA *rna1=new RNA(ctFile.c_str(),1,isRNA);
	...
	if (outFile.empty()) {
		...
		errNum = des->WriteCt(outFile.c_str());//Write a .ct file with the correct sequence
		if (errNum != 0) goto exitProgram; //At bottom of function. deletes des and returns errNum.
	}
...
//	//RNA *rna=new RNA(argv[1],1,IsRNA);
//	//RNA *rna1=new RNA(argv[1],1,IsRNA);
