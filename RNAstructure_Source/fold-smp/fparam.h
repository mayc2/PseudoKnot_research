...
typedef struct fparam
{
  ...
  int ntriloop;
  struct triloop_t { base_t seq[5]; int_t val; } triloop[LOOKUP_TABLE_MAX];
  int ntloop;
  struct tloop_t { base_t seq[6]; int_t val; } tloop[LOOKUP_TABLE_MAX];
  int nhexaloop;
  struct hexaloop_t { base_t seq[8]; int_t val; } hexaloop[LOOKUP_TABLE_MAX];
