...
RNAstructureBackendCalculator::RNAstructureBackendCalculator() {
	...
	// Initialize the progress monitor to a null pointer.
	monitor = 0;
...
//////////////////////////////////////////////////////////////////////////////
string RNAstructureBackendCalculator::buildAllSubDataStructure(
	string file, bool isRNA ) {
	if( rna != 0 ) { delete rna; }
	if( rnaChecker != 0 ) { delete rnaChecker; }
	rna = new RNA( file.c_str(), 2, isRNA );
...
string RNAstructureBackendCalculator::runAllSub(
	...
	if( constraintsHolder.shapeFile != "" ) {
		int shapeErr = rna->ReadSHAPE(
			...
			constraintsHolder.shapeParam1, constraintsHolder.shapeParam2, "SHAPE",
	...
	TProgressDialog* dialog = new TProgressDialog( *(monitor) );
	rna->SetProgress( *dialog );
...
//////////////////////////////////////////////////////////////////////////////
string RNAstructureBackendCalculator::buildBifoldDataStructure(
	string file1, string file2, bool isRNA ) {
	if( hybrid != 0 ) { delete hybrid; }
	if( hybridChecker != 0 ) { delete hybridChecker; }
	hybrid = new HybridRNA( file1.c_str(), 2, file2.c_str(), 2, isRNA );
...
string RNAstructureBackendCalculator::runBifold(
	...
	if( saveFile ) {
		saveFileName =
			ctFile.substr( 0, ctFile.find_last_of( "." ) + 1 ) + "sav";
	}
...
//////////////////////////////////////////////////////////////////////////////
string RNAstructureBackendCalculator::buildBipartitionDataStructure(
	string file1, string file2, bool isRNA ) {
	if( hybrid != 0 ) { delete hybrid; }
	if( hybridChecker != 0 ) { delete hybridChecker; }
	hybrid = new HybridRNA( file1.c_str(), 2, file2.c_str(), 2, isRNA );
...
string RNAstructureBackendCalculator::buildDynalignDataStructure(
	...
	if( dynalignChecker != 0 ) { delete dynalignChecker; }
	dynalign =
		new Dynalign_object( file1.c_str(), 2, file2.c_str(), 2, isRNA );
...
string RNAstructureBackendCalculator::readDynalignAlignmentConstraintsFile(
	...
	while( fileStream.good() ) {
		...
		bool nuc1OK =
			( first >= 1 ) &&
			( first <= dynalign->GetRNA1()->GetSequenceLength() );
		bool nuc2OK =
			( second >= 1 ) &&
			( second <= dynalign->GetRNA2()->GetSequenceLength() );
...
string RNAstructureBackendCalculator::runDynalign(
	...
	int mainError = dynalign->Dynalign(
		structures, windowStr, windowAli, percent, (short)-99, gap, isInsert,
		saveFile.c_str() );
...
//////////////////////////////////////////////////////////////////////////////
string RNAstructureBackendCalculator::buildEfn2DataStructure(
	string file, bool isRNA ) {
	if( rna != 0 ) { delete rna; }
	if( rnaChecker != 0 ) { delete rnaChecker; }
	rna = new RNA( file.c_str(), 1, isRNA );
	rnaChecker = new ErrorChecker<RNA>( rna );
	return rnaChecker->returnError();
}
...
//////////////////////////////////////////////////////////////////////////////
string RNAstructureBackendCalculator::buildFoldDataStructure(
	string file, bool isRNA ) {
	if( rna != 0 ) { delete rna; }
	if( rnaChecker != 0 ) { delete rnaChecker; }
	rna = new RNA( file.c_str(), 2, isRNA );
...
//////////////////////////////////////////////////////////////////////////////
string RNAstructureBackendCalculator::runFold(
...
//////////////////////////////////////////////////////////////////////////////
string RNAstructureBackendCalculator::buildMaxExpectDataStructure(
	string file ) {
	if( rna != 0 ) { delete rna; }
	if( rnaChecker != 0 ) { delete rnaChecker; }
	rna = new RNA( file.c_str(), 3 );
	rnaChecker = new ErrorChecker<RNA>( rna );
	return rnaChecker->returnError();
}
...
string RNAstructureBackendCalculator::buildOligoWalkDataStructure(
	...
	if( oligo != 0 ) { delete oligo; }
	if( oligoChecker != 0 ) { delete oligoChecker; }
	oligo = new Oligowalk_object( file.c_str(), 1 );
	oligo->isrna = isRNA;
	oligoChecker = new ErrorChecker<Oligowalk_object>( oligo );
	return oligoChecker->returnError();
}
...
string RNAstructureBackendCalculator::runOligoWalk(
	...
	oligoDataHolder.deltaG =
		" \u0394G\u00B0<sub>" + temperature.str() + "</sub> = ";
...
//////////////////////////////////////////////////////////////////////////////
string RNAstructureBackendCalculator::buildPartitionDataStructure(
	string file, bool isRNA ) {
	if( rna != 0 ) { delete rna; }
	if( rnaChecker != 0 ) { delete rnaChecker; }
	rna = new RNA( file.c_str(), 2, isRNA );
...
//////////////////////////////////////////////////////////////////////////////
string RNAstructureBackendCalculator::buildProbKnotDataStructure(
	string file ) {
	if( rna != 0 ) { delete rna; }
	if( rnaChecker != 0 ) { delete rnaChecker; }
	rna = new RNA( file.c_str(), 3 );
...
//////////////////////////////////////////////////////////////////////////////
string RNAstructureBackendCalculator::buildRefoldSingleDataStructure(
	string file ) {
	if( rna != 0 ) { delete rna; }
	if( rnaChecker != 0 ) { delete rnaChecker; }
	rna = new RNA( file.c_str(), 4 );
	rnaChecker = new ErrorChecker<RNA>( rna );
	return rnaChecker->returnError();
}
...
void RNAstructureBackendCalculator::addTurboFoldTuple(
	...
	string pfsFile =
		seqFile.substr( 0, seqFile.find_last_of( "." ) ) + ".pfs";
...
string RNAstructureBackendCalculator::runTurboFold(
	...
	int window, double gammaM, int pkIter, int helix, double cutoff ) {
	...
	TurboFold* turbo = new TurboFold( &sequences, &pfsFiles );
...
string RNAstructureBackendCalculator::runTurboFoldPseudoknot(
	double turboGamma, int turboIterations, int pkIterations, int helix ) {

	// Run TurboFold in the workhorse method in ProbKnot mode.
	return runTurboFold(
		turboGamma, turboIterations, "ProbKnot",
		-1, -1, -1, -1, pkIterations, helix, -1 );
}
...
int RNAstructureBackendCalculator::getProgressNumber() {
	...
	return monitor->getMonitorProgress();
}
...
int RNAstructureBackendCalculator::setEnvVar(const string envstr) {
	//cout << "setting env var: " << envstr.c_str()  << endl;
	char* str = strdup(envstr.c_str()); // necessary because putenv on linux is not putenv(const char*), it is putenv(char*)
