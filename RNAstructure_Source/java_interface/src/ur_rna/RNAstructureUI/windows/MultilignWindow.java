...
public class MultilignWindow extends MultiWindow {
	...
	protected void buildSpecificOptionsPanel() {
		...
		// This is placed inside a block for organizational purposes.
		{
			...
			NumberPanel suboptimal = new NumberPanel(
		...
		NumberPanel multilign = new NumberPanel( iterations, maxPairs, dsv );
	...
	protected void executeModuleAction( String command ) {
		...
		NumberPanel multilignPanel = (NumberPanel)getInputControl( tabBox, 1 );
		...
		else if( command.equals( "Alignment File" ) ) {
			int index = 1;
	...
	protected void runMainCalculationMultipleSequences() {
		...
		if( align.isError() ) { return; }

		// Get the suboptimal structure parameters.
		Box dynalignTab = (Box)getInputControl( tabs, 1 );
		NumberPanel suboptimal =
			(NumberPanel)getInputControl( dynalignTab, 1 );
		...
		NumberPanel multilign =
			(NumberPanel)getInputControl( multilignTab, 1 );
