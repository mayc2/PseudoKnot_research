...
public class OligoWalkWindow
	...
	private void buildResultsWindow(
		...
		isInputWindow = false;
		setTitles( "OligoWalk Results: " + file );
		menuBar = new DynamicMenuBar( this, setMenus() );
		menuBar.enableMenus();
		ApplicationRootFrame.getFrame().setJMenuBar( menuBar );
		...
		addKeyListener( new KeyAdapter() {
			...
			public void keyPressed( KeyEvent e ) {
				...
				executeModuleAction( "Move Oligo " + Integer.toString( move ) );
			}
		});
		...
		String initType = ((OligoWalkResultsMenu)menuBar.getMenu( 3 ))
			.getItem( 0 ).getText();
		String stopString = Integer.toString( stop );
		panelResults.setName( initType + ";0/" + stopString );
		executeModuleAction( "Menu " + initType );
		setOligoData( backend.getGraphRegionBegin() );

		// Revalidate and repaint the menu bar.
		menuBar.revalidate();
		menuBar.repaint();
	}
	...
	private void createInputPanel() {
		...
		oligoLength.getSpinner().addChangeListener( new ChangeListener() {
			public void stateChanged( ChangeEvent e ) {
				executeModuleAction( "Update Target Region Range" );
			}
		});

		// Create the oligo concentration panel.
		IntegerField conc = new IntegerField( "Oligo Concentration", 1, 1 );
		NumberPanel concField = new NumberPanel( conc );
		concField.setPanelWidth( 200 );
		concField.makePanel();
		StringComboBox concBox = new StringComboBox( "mM", "uM", "nM", "pM" );
	...
	private void createResultsPanel() {
		...
		oligo.addMouseListener( new MouseAdapter() {
			...
			public void mousePressed( MouseEvent e ) {
				...
				if( ( oligo == false ) && ( self == false ) ) { return; }
				...
				DrawingWindow temp = new DrawingWindow( tempFile );
				if( temp.isError() == false ) { temp.viewWindow(); }
	...
	protected void executeModuleAction( String command ) {
		...
		if( isInputWindow ) {
			...
			else if( command.equals( "Update Target Region Range" ) ) {
		...
		// Otherwise, do any actions for the results window.
			...
			if( command.startsWith( "Menu" ) ) {
				...
				String panelType = command.substring( 5 );
				...
				if( command.endsWith( "Overall and Duplex" ) ) {
					color1 = Color.BLUE;
			...
			// next proper oligo.
			else if( command.startsWith( "Move Oligo" ) ) {
				String name = panelResults.getName();
				String currentString = name.substring(
					name.indexOf( ";" ) + 1, name.indexOf( "/" ) );
				int currentIndex = Integer.parseInt( currentString );
				int moveAmount = Integer.parseInt( command.split( " " )[2] );
			...
			else if( command.startsWith( "Set Oligo" ) ) {
	...
	protected void runMainCalculation() {
		...
		Integer concAmount =
			((IntegerField)((NumberPanel)getInputControl( concPanel, 1 ))
			.getField( 1 )).getValue();
		String concUnit =
	...
	protected RolloverMenu[] setMenus() {
		...
		if( isInputWindow ) {
			...
			return new RolloverMenu[]{ resultsMenu };
		}
	}
	...
	class OligoWalkResultsMenu
		...
		public OligoWalkResultsMenu() {
			...
			addSeparator();
			addMenuItem( "Overall", "Show overall oligo results." );
			addMenuItem( "Duplex", "Show duplex oligo results." );
			addMenuItem(
			...
			for( Component component: components ) {
				if( component instanceof JMenuItem ) {
					String action = "Menu " + component.getName();
					((JMenuItem)component).setActionCommand( action );
				}
			}
		}
